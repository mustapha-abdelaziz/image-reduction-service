...
const redactionController: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.post('/redact', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      // Get multipart data
      ...
      try {
        const parsed = typeof opsField === 'string' ? JSON.parse(opsField) : opsField;
      ...
      // Process the image
      const { result: processed, durationMs } = await measureAsync(async () => {
        return redactionService.processMultipartRequest(fileBuffer, requestData, mimeType);
      });

      // Set response headers
      reply.header('Content-Type', processed.format === 'webp' ? 'image/webp' : 'image/jpeg');
   ...
   */
  fastify.post('/redact/s3', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const { result, durationMs } = await measureAsync(async () => {
        ...
        const idempotencyKey = requestData.idempotency_key || generateRequestHash({
        ...
        if (outputExists && requestData.idempotency_key) {
          request.log.info({ idempotencyKey }, 'Output already exists, skipping processing');
