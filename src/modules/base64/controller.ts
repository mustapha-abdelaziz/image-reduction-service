...
import { RedactionService, RedactionServiceError } from '@/modules/redaction/service.js';
import { z } from 'zod';
...
function convertToInternalRegion(simple: z.infer<typeof simpleRegionSchema>): Region {
  const coords = simple.coords;

  // Normalize coordinates if they're > 1 (pixel coordinates)
  const isNormalized = coords.x <= 1 && coords.y <= 1 && coords.width <= 1 && coords.height <= 1;
...
const base64Controller: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.post('/redact/base64', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      try {
        ...
        request.log.error({ error, traceId: request.traceId }, 'Failed to read image metadata');
        return reply.status(400).send({
          code: 'INVALID_IMAGE',
          message: 'Unable to process image. Please ensure the base64 data is a valid image.',
          traceId: request.traceId,
        });
      }
      ...
      for (let i = 0; i < validated.regions.length; i++) {
        ...
        if (isNormalized) {
          ...
          coords.width = Math.max(1, Math.min(imgWidth - coords.x, coords.width));
          coords.height = Math.max(1, Math.min(imgHeight - coords.y, coords.height));

          // Log adjustment if coordinates were clamped
          if (originalCoords.x !== coords.x || originalCoords.y !== coords.y ||
              originalCoords.width !== coords.width || originalCoords.height !== coords.height) {
            request.log.info({
              region: i + 1,
      ...
      const regions: Region[] = validated.regions.map(convertToInternalRegion);

      // Process image with timing
      const { result: outputBuffer, durationMs } = await measureAsync(async () => {
        return await redactionService.redactImage(
          imageBuffer,
          regions,
          {
      ...
      reply.header('Content-Disposition', `attachment; filename="redacted.${format}"`);
      ...
      request.log.error({ error, traceId: request.traceId }, 'Base64 redaction error');
